...
import com.thebombzen.zengifr.util.io.resources.ResourceNotFoundException;
...
public final class LibraryLoader {
	...
	public static void main(String[] args) throws Exception {
		...
		if (thisJar.getFileName().toString().endsWith(".jar")) {
			...
			newArgs.add("-cp");
			newArgs.add(cp);
			newArgs.add("com.thebombzen.zengifr.ZenGIFr");
			newArgs.addAll(Arrays.asList(args));
			ProcessBuilder builder = new ProcessBuilder(newArgs);
			builder.inheritIO().start();
			System.exit(0);
		} else {
			Class<?> clazz = Class.forName("com.thebombzen.zengifr.ZenGIFr", false,
